    ...
    <div style={{position:"relative",flexShrink:0}}>
      ...
      {open&&(
        <div style={{position:"fixed",zIndex:1000,background:tk.bgCard,border:"0.5px solid "+tk.gray4,borderRadius:tk.radius,padding:"10px",display:"flex",gap:8,flexWrap:"wrap",width:148,boxShadow:"0 8px 24px rgba(0,0,0,0.12)"}}>
          {COLORS.map(c=><button key={c} onClick={()=>{onChange(c);setOpen(false);}} style={{width:22,height:22,borderRadius:"50%",background:c,border:"none",cursor:"pointer",padding:0,outline:value===c?"2.5px solid "+c:"none",outlineOffset:2}}/>)}
        </div>
      )}
    </div>
  );
...
function GroupCard({children}){return <div style={{background:tk.bgCard,borderRadius:tk.radiusLg,overflow:"hidden",marginBottom:8}}>{children}</div>;}
function Row({children,last}){return <div style={{display:"flex",alignItems:"center",gap:12,padding:"11px 16px",minHeight:44,textAlign:"left",borderBottom:last?"none":"0.5px solid "+tk.gray5}}>{children}</div>;}
function SectionHeader({children}){return <div style={{fontSize:12,fontWeight:600,color:tk.gray1,textTransform:"uppercase",letterSpacing:"0.05em",padding:"20px 4px 6px",display:"flex",alignItems:"center",gap:6}}>{children}</div>;}
function Badge({color,children}){return <span style={{display:"inline-flex",alignItems:"center",height:18,padding:"0 7px",borderRadius:tk.radiusPill,fontSize:11,fontWeight:600,background:color+"20",color}}>{children}</span>;}
...
export default function App() {
      ...
      return(
        <div key={g.key}>
          <SectionHeader>
            ...
            {proj&&proj.open_date&&<span style={{fontSize:11,color:tk.gray1,fontWeight:400,textTransform:"none",letterSpacing:0}}>{fmtDate(proj.open_date)}{" 오픈"}</span>}
          </SectionHeader>
                ...
                return(
                  <div key={ph.id} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 16px",minHeight:44,borderBottom:i===a.length-1?"none":"0.5px solid "+tk.gray5}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{fontSize:12,color:tk.gray1}}>{fmtDate(ph.start_date)}{" ~ "}{fmtDate(ph.end_date)}</div>
                    </div>
                    <button onClick={()=>{setEditPhaseId(ph.id);setSelProj(String(ph.project_id));setPhaseForm({name:ph.name,start_date:ph.start_date,end_date:ph.end_date,color:ph.color});setShowPhaseForm(true);}} style={sel}>{"수정"}</button>
  ...
  if(loading) return <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",fontFamily:font,color:tk.gray1,fontSize:15}}>{"불러오는 중..."}</div>;
        ...
        {showProjMgr&&(
          <>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <button onClick={()=>setShowNewProjForm(v=>!v)} style={{background:"none",border:"none",color:tk.blue,fontSize:13,cursor:"pointer",fontFamily:font,padding:"0 4px"}}>{showNewProjForm?"접기":"+ 추가"}</button>
            </div>
            <GroupCard>
                  ...
                  {editingProjId===p.id?(
                    ...
                    <>
                      <div style={{flex:1,minWidth:0,textAlign:"left"}}>
                        ...
                        {p.open_date&&<div style={{fontSize:12,color:tk.gray1,marginTop:2}}>{fmtDate(p.open_date)}{" 오픈"}</div>}
                      </div>
                      ...
                      <button onClick={()=>{setEditingProjId(p.id);setEditingProjName(p.name);setEditingProjColor(p.color);setEditingProjOpenDate(p.open_date||"");}} style={{...sel,flexShrink:0}}>{"수정"}</button>
                  ...
                  {projects.filter(p=>p.done).map((p,i,a)=>(
                    <div key={p.id} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 16px",minHeight:44,borderBottom:i===a.length-1?"none":"0.5px solid "+tk.gray5}}>
                      ...
                      <div style={{flex:1,minWidth:0,opacity:0.5,textAlign:"left"}}>
                        ...
                        {p.open_date&&<div style={{fontSize:12,color:tk.gray2,marginTop:2}}>{fmtDate(p.open_date)}{" 오픈"}</div>}
                      </div>
