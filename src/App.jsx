...
function DraggableCalendar({ phases, projects, calDate, setCalDate, onUpdatePhase }) {
  const [dragging, setDragging] = useState(null); // { phaseId, startDayStr, origStart, origEnd }
  ...
  const visiblePhases = phases.filter(ph => ph.start?.slice(0, 7) <= monthStr && ph.end?.slice(0, 7) >= monthStr);
  ...
  function handlePhaseDragStart(e, ph) {
    e.stopPropagation();
    const dayStr = e.currentTarget.closest("[data-day]")?.dataset.day;
    setDragging({ phaseId: ph.id, startDayStr: dayStr, origStart: ph.start, origEnd: ph.end });
    e.dataTransfer.effectAllowed = "move";
    e.dataTransfer.setData("text/plain", ph.id);
  }

  function handleDayDragOver(e, dayStr) {
    e.preventDefault();
    e.dataTransfer.dropEffect = "move";
    setHoverDay(dayStr);
  }

  function handleDayDrop(e, dayStr) {
    e.preventDefault();
    if (!dragging) return;
    const delta = diffDays(dragging.startDayStr, dayStr);
    if (delta !== 0) {
      onUpdatePhase(dragging.phaseId, {
        start: addDays(dragging.origStart, delta),
        end: addDays(dragging.origEnd, delta),
      });
    }
    setDragging(null);
    setHoverDay(null);
  ...
  function getPreviewDates(ph) {
    if (!dragging || dragging.phaseId !== ph.id || !hoverDay || !dragging.startDayStr) return { start: ph.start, end: ph.end };
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
        <button onClick={() => setCalDate(({ y, m }) => m === 0 ? { y: y-1, m: 11 } : { y, m: m-1 })}
          style={{ background: "none", border: "none", fontSize: 22, cursor: "pointer", color: tk.blue, padding: "0 8px" }}>‹</button>
        <span style={{ fontSize: 16, fontWeight: 600, color: tk.label }}>{y}년 {m + 1}월</span>
        <button onClick={() => setCalDate(({ y, m }) => m === 11 ? { y: y+1, m: 0 } : { y, m: m+1 })}
          style={{ background: "none", border: "none", fontSize: 22, cursor: "pointer", color: tk.blue, padding: "0 8px" }}>›</button>
      </div>

      {/* 요일 헤더 */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", marginBottom: 4 }}>
        {dayNames.map(d => <div key={d} style={{ textAlign: "center", fontSize: 11, fontWeight: 600, color: tk.gray1, padding: "4px 0" }}>{d}</div>)}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 2 }}>
        {cells.map((d, i) => {
          ...
          // 이 날짜에 걸쳐 있는 단계들 (드래그 중이면 미리보기 반영)
          const dayPhases = visiblePhases.filter(ph => {
            const { start, end } = getPreviewDates(ph);
          ...
          return (
            <div
              ...
              style={{
                minHeight: 64, padding: "4px 3px",
                background: isHover ? tk.blueLight : isToday ? "#F0F7FF" : "transparent",
                borderRadius: 8,
            ...
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                {dayPhases.slice(0, 3).map(ph => {
                  const { start, end } = getPreviewDates(ph);
                  const isStart = start === dateStr;
                  const isEnd = end === dateStr;
                  const isDraggingThis = dragging?.phaseId === ph.id;
                  return (
                    <div
                      key={ph.id}
                      draggable
                      ...
                      title={`${ph.name} (${fmtDate(ph.start)}~${fmtDate(ph.end)}) — 드래그로 이동`}
                      style={{
                        background: ph.color,
                        color: "#fff",
...
export default function App() {
  ...
  function submitPhase() {
    ...
    if (editPhaseId) {
      setPhases(ps => ps.map(p => p.id === editPhaseId ? { ...p, ...phaseForm, projectId: Number(selectedProjForPhase) } : p));
      setEditPhaseId(null);
    } else {
      setPhases(ps => [...ps, { ...phaseForm, id: Date.now(), projectId: Number(selectedProjForPhase) }]);
    }
  ...
  function renderTask(t, idx, arr) {
    return (
      <Row key={t.id} last={idx === arr.length - 1}>
        <button onClick={() => toggleDone(t.id)} style={{ width: 22, height: 22, borderRadius: "50%", border: t.done ? "none" : `1.5px solid ${tk.gray3}`, background: t.done ? tk.blue : "transparent", flexShrink: 0, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", padding: 0 }}>
      ...
      return (
        <div key={key}>
          <SectionHeader>
            ...
            <span style={{ background: tk.gray5, color: tk.gray1, borderRadius: tk.radiusPill, padding: "1px 7px", fontSize: 11, fontWeight: 600, textTransform: "none", letterSpacing: 0 }}>{gtasks.length}</span>
            {proj?.openDate && <span style={{ fontSize: 11, color: tk.gray1, fontWeight: 400, textTransform: "none", letterSpacing: 0 }}>{fmtDate(proj.openDate)} 오픈</span>}
          </SectionHeader>
          <GroupCard>{gtasks.map((t, i, a) => renderTask(t, i, a))}</GroupCard>
        </div>
      );
    });
  }
  ...
  function renderScheduleTab() {
    const filteredPhases = phases.filter(p => !selectedProjForPhase || p.projectId === Number(selectedProjForPhase));
    return (
      <div>
        ...
        <GroupCard>
          <Row last>
            ...
            <select value={selectedProjForPhase} onChange={e => setSelectedProjForPhase(e.target.value)} style={selStyle}>
        ...
        {selectedProjForPhase && (
          <>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <SectionHeader>단계 추가</SectionHeader>
              ...
              <>
                <GroupCard>
                  ...
                  <Row>
                    ...
                    <input type="date" value={phaseForm.start} onChange={e => setPhaseForm(f => ({ ...f, start: e.target.value }))} style={selStyle} />
                  </Row>
                  <Row last>
                    <span style={{ flex: 1, fontSize: 15 }}>종료일</span>
                    <input type="date" value={phaseForm.end} onChange={e => setPhaseForm(f => ({ ...f, end: e.target.value }))} style={selStyle} />
                  </Row>
                </GroupCard>
                <div style={{ margin: "0 0 16px" }}>
                  <button onClick={() => { submitPhase(); setShowPhaseForm(false); }} style={{ width: "100%", background: tk.blue, color: "#fff", border: "none", borderRadius: tk.radius, height: 44, fontSize: 15, fontWeight: 500, cursor: "pointer", fontFamily: font }}>
        ...
        {filteredPhases.length > 0 && (
          <>
            ...
            <GroupCard>
              {filteredPhases.map((ph, i, a) => {
                const proj = projOf(ph.projectId);
                return (
                  <Row key={ph.id} last={i === a.length - 1}>
                    <span style={{ width: 10, height: 10, borderRadius: 3, background: ph.color, flexShrink: 0 }} />
                    ...
                    <button onClick={() => { setEditPhaseId(ph.id); setSelectedProjForPhase(String(ph.projectId)); setPhaseForm({ name: ph.name, start: ph.start, end: ph.end, color: ph.color }); }} style={selStyle}>수정</button>
                    <button onClick={() => deletePhase(ph.id)} style={{ ...selStyle, color: tk.red }}>삭제</button>
                  </Row>
                );
              })}
            </GroupCard>
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "0 16px 40px" }}>
        {showProjMgr && (
          <>
            ...
            <GroupCard>
              {projects.length === 0 && <Row last><span style={{ color: tk.gray2, fontSize: 15 }}>등록된 과제가 없어요</span></Row>}
              {projects.map((p, i, a) => (
                <Row key={p.id} last={i === a.length - 1}>
                  ...
                  {editingProjId === p.id ? (
                    <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <input type="date" value={editingProjOpenDate} onChange={e => setEditingProjOpenDate(e.target.value)} style={{ ...selStyle, fontSize: 13 }} />
                      </div>
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={() => saveEditProj(p.id)} style={selStyle}>저장</button>
                        <button onClick={() => setEditingProjId(null)} style={{ ...selStyle, color: tk.gray1 }}>취소</button>
                      </div>
                    </div>
                  ) : (
                    <>
              ...
              <Row last>
                ...
                <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 4 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    <input type="date" value={newProjOpenDate} onChange={e => setNewProjOpenDate(e.target.value)} style={{ ...selStyle, fontSize: 12 }} />
                  </div>
                </div>
                <button onClick={addProject} style={{ background: tk.blue, color: "#fff", border: "none", borderRadius: 8, padding: "0 14px", height: 32, fontSize: 14, cursor: "pointer", fontFamily: font }}>추가</button>
              </Row>
            </GroupCard>
          </>
        )}
        ...
        {showForm && (
          <>
            ...
            <GroupCard>
              ...
              <Row>
                <span style={{ flex: 1, fontSize: 15 }}>과제</span>
                <select value={form.projectId} onChange={e => setForm(f => ({ ...f, projectId: e.target.value ? Number(e.target.value) : "" }))} style={selStyle}>
              ...
              <Row>
                ...
                <select value={form.schedule} onChange={e => setForm(f => ({ ...f, schedule: e.target.value, scheduleDate: "" }))} style={selStyle}>
              ...
              {form.schedule === "date" && (
                <Row>
                  ...
                  <input type="date" value={form.scheduleDate} min={todayStr} onChange={e => setForm(f => ({ ...f, scheduleDate: e.target.value }))} style={selStyle} />
                </Row>
              )}
              <Row last>
                <span style={{ flex: 1, fontSize: 15 }}>마감일</span>
                <input type="date" value={form.due} onChange={e => setForm(f => ({ ...f, due: e.target.value }))} style={{ ...selStyle, color: form.due ? tk.blue : tk.gray2 }} />
              </Row>
            </GroupCard>
