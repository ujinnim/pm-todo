...
const tk = {
  ...
  gray4: "#D1D1D6", gray5: "#E5E5EA", blueLight: "#EBF2FF",
  label: "#000000", bg: "#F2F2F7", bgCard: "#FFFFFF",
...
function ColorPicker({ value, onChange }) {
  ...
  return (
    <div style={{ position: "relative", flexShrink: 0 }}>
      <button onClick={() => setOpen(function(v) { return !v; })} style={{ width: 24, height: 24, borderRadius: "50%", background: value, border: "2px solid rgba(0,0,0,0.08)", cursor: "pointer", padding: 0 }} />
      {open && (
...
function Badge({ color, children }) {
  return <span style={{ display: "inline-flex", alignItems: "center", height: 18, padding: "0 7px", borderRadius: tk.radiusPill, fontSize: 11, fontWeight: 600, background: color + "20", color: color }}>{children}</span>;
}
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
        <button onClick={() => setCalDate(function(c) { return c.m === 0 ? { y: c.y-1, m: 11 } : { y: c.y, m: c.m-1 }; })} style={{ background: "none", border: "none", fontSize: 22, cursor: "pointer", color: tk.blue, padding: "0 8px" }}>‹</button>
        <span style={{ fontSize: 16, fontWeight: 600, color: tk.label }}>{y}년 {m + 1}월</span>
        <button onClick={() => setCalDate(function(c) { return c.m === 11 ? { y: c.y+1, m: 0 } : { y: c.y, m: c.m+1 }; })} style={{ background: "none", border: "none", fontSize: 22, cursor: "pointer", color: tk.blue, padding: "0 8px" }}>›</button>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", marginBottom: 4 }}>
        {dayNames.map(function(d) { return <div key={d} style={{ textAlign: "center", fontSize: 11, fontWeight: 600, color: tk.gray1, padding: "4px 0" }}>{d}</div>; })}
      </div>
          ...
          return (
            <div key={d} data-day={dateStr}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                ...
                {dayPhases.length > 3 && <div style={{ fontSize: 9, color: tk.gray1, textAlign: "center" }}>+{dayPhases.length - 3}</div>}
              </div>
            </div>
          );
        })}
...
export default function App() {
  const [tasks, setTasks] = useState(function() { return load(TASKS_KEY, []); });
  const [projects, setProjects] = useState(function() { return load(PROJECTS_KEY, []); });
  const [phases, setPhases] = useState(function() { return load(PHASES_KEY, []); });
  const [appTitle, setAppTitle] = useState(function() { return load(TITLE_KEY, "할 일"); });
  ...
  const [calDate, setCalDate] = useState(function() { const d = new Date(); return { y: d.getFullYear(), m: d.getMonth() }; });
  const [selectedProjForPhase, setSelectedProjForPhase] = useState("");
  ...
  useEffect(function() { save(TASKS_KEY, tasks); }, [tasks]);
  useEffect(function() { save(PROJECTS_KEY, projects); }, [projects]);
  useEffect(function() { save(PHASES_KEY, phases); }, [phases]);
  useEffect(function() { save(TITLE_KEY, appTitle); }, [appTitle]);

  const todayStr = today();
  const isOverdue = function(d) { return d && d < todayStr; };
  const isDueToday = function(d) { return d === todayStr; };
  const isDueSoon = function(d) { if (!d) return false; const diff = (new Date(d) - new Date(todayStr)) / 86400000; return diff > 0 && diff <= 3; };
  const projOf = function(id) { return projects.find(function(p) { return p.id === id; }); };
  const selStyle = { fontSize: 14, border: "none", background: "transparent", color: tk.blue, outline: "none", cursor: "pointer", fontFamily: font };
  ...
  function deleteProject(id) {
    ...
    setTasks(function(ts) { return ts.map(function(t) { return t.projectId === id ? Object.assign({}, t, { projectId: "" }) : t; }); });
  }
  function toggleProjDone(id) {
    setProjects(function(ps) {
      return ps.map(function(p) {
        ...
        return Object.assign({}, p, { done: !p.done });
      });
    });
  }
  function saveEditProj(id) {
    const name = editingProjName.trim();
    if (name) setProjects(function(ps) { return ps.map(function(p) { return p.id === id ? Object.assign({}, p, { name: name, color: editingProjColor, openDate: editingProjOpenDate }) : p; }); });
    setEditingProjId(null);
  }
  function submit() {
    if (!form.title.trim()) return;
    const task = Object.assign({}, form);
    if (task.schedule !== "date") task.scheduleDate = "";
    if (editId) {
      setTasks(function(ts) { return ts.map(function(t) { return t.id === editId ? Object.assign({}, t, task) : t; }); });
      setEditId(null);
    } else {
      setTasks(function(ts) { return [...ts, Object.assign({}, task, { id: Date.now(), done: false, createdAt: todayStr })]; });
    }
    setForm(function(f) { return { title: "", memo: "", due: "", schedule: "today", scheduleDate: "", projectId: f.projectId }; });
    setShowForm(false);
  }
  ...
  function submitPhase() {
    if (!phaseForm.name.trim() || !phaseForm.start || !phaseForm.end || !selectedProjForPhase) return;
    if (editPhaseId) {
      setPhases(function(ps) { return ps.map(function(p) { return p.id === editPhaseId ? Object.assign({}, p, phaseForm, { projectId: Number(selectedProjForPhase) }) : p; }); });
      setEditPhaseId(null);
    } else {
      setPhases(function(ps) { return [...ps, Object.assign({}, phaseForm, { id: Date.now(), projectId: Number(selectedProjForPhase) })]; });
    }
  ...
  function deletePhase(id) {
    ...
    setPhases(function(ps) { return ps.filter(function(p) { return p.id !== id; }); });
  }
  function updatePhase(id, updates) {
    setPhases(function(ps) { return ps.map(function(p) { return p.id === id ? Object.assign({}, p, updates) : p; }); });
  }

  const undone = tasks.filter(function(t) { return !t.done; });
  const done = tasks.filter(function(t) { return t.done; });
  const todayCount = undone.filter(function(t) { return getEffectiveSchedule(t, todayStr) === "today"; }).length;
  const backlogCount = undone.filter(function(t) { return getEffectiveSchedule(t, todayStr) === "backlog"; }).length;

  function getFiltered() {
    if (view === "today") return undone.filter(function(t) { return getEffectiveSchedule(t, todayStr) === "today"; });
    if (view === "backlog") return undone.filter(function(t) { return getEffectiveSchedule(t, todayStr) === "backlog"; });
  ...
  function groupByProject(list) {
    ...
    const order = projects.map(function(p) { return String(p.id); }).concat(["__none__"]);
    return order.filter(function(k) { return groups[k]; }).map(function(k) { return { key: k, tasks: groups[k] }; });
  }
      ...
      <div key={t.id} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "11px 16px", minHeight: 44, borderBottom: idx === arr.length - 1 ? "none" : "0.5px solid " + tk.gray5 }}>
        ...
        <div style={{ flex: 1, minWidth: 0, opacity: t.done ? 0.4 : 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
            ...
            {t.schedule === "date" && t.scheduleDate && <Badge color={t.scheduleDate <= todayStr ? tk.orange : tk.blue}>{fmtDate(t.scheduleDate)}</Badge>}
            {t.due && (isOverdue(t.due) ? <Badge color={tk.red}>{fmtDate(t.due)} 초과</Badge> : isDueToday(t.due) ? <Badge color={tk.orange}>오늘 마감</Badge> : isDueSoon(t.due) ? <Badge color={tk.orange}>{"~"+fmtDate(t.due)}</Badge> : <Badge color={tk.gray1}>{"~"+fmtDate(t.due)}</Badge>)}
          </div>
    ...
    return groups.map(function(g) {
      ...
      return (
        <div key={g.key}>
          ...
          <GroupCard>{g.tasks.map(function(t, i, a) { return renderTask(t, i, a); })}</GroupCard>
        </div>
      );
    });
  }
  ...
  function renderScheduleTab() {
    const filteredPhases = phases.filter(function(p) { return !selectedProjForPhase || p.projectId === Number(selectedProjForPhase); });
    return (
      <div>
        ...
        <GroupCard>
          <Row last>
            ...
            <select value={selectedProjForPhase} onChange={function(e) { setSelectedProjForPhase(e.target.value); }} style={selStyle}>
              <option value="">전체 보기</option>
              {projects.map(function(p) { return <option key={p.id} value={p.id}>{p.name}</option>; })}
            </select>
          </Row>
        </GroupCard>
        {selectedProjForPhase && (
          <>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <SectionHeader>단계</SectionHeader>
              <button onClick={() => setShowPhaseForm(function(v) { return !v; })} style={{ background: "none", border: "none", color: tk.blue, fontSize: 13, cursor: "pointer", fontFamily: font, padding: "0 4px" }}>
                {showPhaseForm ? "접기" : "+ 추가"}
              </button>
            </div>
            {showPhaseForm && (
              <>
                <GroupCard>
                  <Row>
                    <input placeholder="단계 이름 (기획, 디자인, 개발...)" value={phaseForm.name}
                      onChange={function(e) { setPhaseForm(function(f) { return Object.assign({}, f, { name: e.target.value }); }); }}
                      style={{ flex: 1, fontSize: 15, border: "none", outline: "none", background: "transparent", fontFamily: font }} />
                    <ColorPicker value={phaseForm.color} onChange={function(c) { setPhaseForm(function(f) { return Object.assign({}, f, { color: c }); }); }} />
                  </Row>
                  <Row>
                    <span style={{ flex: 1, fontSize: 15 }}>시작일</span>
                    <input type="date" value={phaseForm.start} onChange={function(e) { setPhaseForm(function(f) { return Object.assign({}, f, { start: e.target.value }); }); }} style={selStyle} />
                  </Row>
                  <Row last>
                    <span style={{ flex: 1, fontSize: 15 }}>종료일</span>
                    <input type="date" value={phaseForm.end} onChange={function(e) { setPhaseForm(function(f) { return Object.assign({}, f, { end: e.target.value }); }); }} style={selStyle} />
                  </Row>
                </GroupCard>
        ...
        {filteredPhases.length > 0 && (
          <>
            ...
            <GroupCard>
              {filteredPhases.map(function(ph, i, a) {
                ...
                return (
                  <div key={ph.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 16px", minHeight: 44, borderBottom: i === a.length - 1 ? "none" : "0.5px solid " + tk.gray5 }}>
                    ...
                    <button onClick={() => { setEditPhaseId(ph.id); setSelectedProjForPhase(String(ph.projectId)); setPhaseForm({ name: ph.name, start: ph.start, end: ph.end, color: ph.color }); setShowPhaseForm(true); }} style={selStyle}>수정</button>
                    <button onClick={() => deletePhase(ph.id)} style={Object.assign({}, selStyle, { color: tk.red })}>삭제</button>
                  </div>
                );
              })}
            </GroupCard>
        ...
        <GroupCard>
          <div style={{ padding: 16 }}>
            <DraggableCalendar phases={filteredPhases} projects={projects} calDate={calDate} setCalDate={setCalDate} onUpdatePhase={updatePhase} />
          </div>
        </GroupCard>
      </div>
    );
  ...
  return (
    <div style={{ background: tk.bg, minHeight: "100vh", fontFamily: font }}>
      ...
      <div style={{ background: "rgba(255,255,255,0.85)", backdropFilter: "blur(20px)", borderBottom: "0.5px solid " + tk.gray4, padding: "0 20px", height: 52, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100 }}>
        {editingTitle ? (
          <input value={appTitle} onChange={function(e) { setAppTitle(e.target.value); }}
            onBlur={() => setEditingTitle(false)}
            onKeyDown={function(e) { if (e.key === "Enter" || e.key === "Escape") setEditingTitle(false); }}
            autoFocus style={{ fontSize: 17, fontWeight: 600, border: "none", borderBottom: "1.5px solid " + tk.blue, outline: "none", background: "transparent", width: 180, fontFamily: font }} />
        ) : (
          <span onClick={() => setEditingTitle(true)} style={{ fontSize: 17, fontWeight: 600, cursor: "pointer" }}>{appTitle}</span>
        )}
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => setShowProjMgr(function(v) { return !v; })} style={{ background: tk.bgCard, color: tk.blue, border: "0.5px solid " + tk.gray4, borderRadius: tk.radius, padding: "0 14px", height: 34, fontSize: 14, cursor: "pointer", fontFamily: font }}>과제 관리</button>
          <button onClick={() => { setShowForm(function(v) { return !v; }); setEditId(null); }} style={{ background: tk.blue, color: "#fff", border: "none", borderRadius: tk.radius, padding: "0 14px", height: 34, fontSize: 14, fontWeight: 500, cursor: "pointer", fontFamily: font }}>+ 추가</button>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "0 16px 40px" }}>
        {showProjMgr && (
          <>
            ...
            <GroupCard>
              ...
              {projects.filter(function(p) { return !p.done; }).map(function(p, i, a) {
                return (
                    ...
                    {editingProjId === p.id ? (
                      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          ...
                          <input value={editingProjName} onChange={function(e) { setEditingProjName(e.target.value); }}
                            onKeyDown={function(e) { if (e.key === "Enter") { e.preventDefault(); saveEditProj(p.id); } else if (e.key === "Escape") setEditingProjId(null); }}
                            autoFocus style={{ flex: 1, fontSize: 15, border: "none", outline: "none", fontFamily: font }} />
                        </div>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <span style={{ fontSize: 13, color: tk.gray1 }}>오픈일</span>
                          <input type="date" value={editingProjOpenDate} onChange={function(e) { setEditingProjOpenDate(e.target.value); }} style={Object.assign({}, selStyle, { fontSize: 13 })} />
                        </div>
                        <div style={{ display: "flex", gap: 8 }}>
                          <button onClick={() => saveEditProj(p.id)} style={selStyle}>저장</button>
                          <button onClick={() => setEditingProjId(null)} style={Object.assign({}, selStyle, { color: tk.gray1 })}>취소</button>
                        </div>
                      </div>
                    ) : (
                      <>
                        ...
                        <button onClick={() => { setEditingProjId(p.id); setEditingProjName(p.name); setEditingProjColor(p.color); setEditingProjOpenDate(p.openDate || ""); }} style={Object.assign({}, selStyle, { flexShrink: 0 })}>수정</button>
                        <button onClick={() => deleteProject(p.id)} style={Object.assign({}, selStyle, { color: tk.red, flexShrink: 0 })}>삭제</button>
                      </>
                    )}
                  </div>
                );
            ...
            {projects.filter(function(p) { return p.done; }).length > 0 && (
              <>
                  ...
                  {projects.filter(function(p) { return p.done; }).map(function(p, i, a) {
                    return (
                      <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 16px", minHeight: 44, borderBottom: i === a.length - 1 ? "none" : "0.5px solid " + tk.gray5 }}>
                        ...
                        <button onClick={() => toggleProjDone(p.id)} style={Object.assign({}, selStyle, { flexShrink: 0 })}>되돌리기</button>
                        <button onClick={() => deleteProject(p.id)} style={Object.assign({}, selStyle, { color: tk.red, flexShrink: 0 })}>삭제</button>
                      </div>
                    );
                  })}
            ...
            <GroupCard>
              <Row>
                <input placeholder="할 일을 입력하세요" value={form.title}
                  ...
                  style={{ flex: 1, fontSize: 16, border: "none", outline: "none", background: "transparent", fontFamily: font }} />
              </Row>
              <Row>
                <textarea placeholder="메모 (선택)" value={form.memo}
                  onChange={function(e) { setForm(function(f) { return Object.assign({}, f, { memo: e.target.value }); }); }}
                  rows={2} style={{ flex: 1, fontSize: 14, border: "none", outline: "none", resize: "none", background: "transparent", fontFamily: font, lineHeight: 1.5, color: tk.label }} />
              </Row>
              <Row>
                <span style={{ flex: 1, fontSize: 15 }}>과제</span>
                <select value={form.projectId} onChange={function(e) { setForm(function(f) { return Object.assign({}, f, { projectId: e.target.value ? Number(e.target.value) : "" }); }); }} style={selStyle}>
              ...
              <Row>
                ...
                <select value={form.schedule} onChange={function(e) { setForm(function(f) { return Object.assign({}, f, { schedule: e.target.value, scheduleDate: "" }); }); }} style={selStyle}>
              ...
              {form.schedule === "date" && (
                <Row>
                  ...
                  <input type="date" value={form.scheduleDate} min={todayStr} onChange={function(e) { setForm(function(f) { return Object.assign({}, f, { scheduleDate: e.target.value }); }); }} style={selStyle} />
                </Row>
              )}
              <Row last>
                <span style={{ flex: 1, fontSize: 15 }}>마감일</span>
                <input type="date" value={form.due} onChange={function(e) { setForm(function(f) { return Object.assign({}, f, { due: e.target.value }); }); }} style={Object.assign({}, selStyle, { color: form.due ? tk.blue : tk.gray2 })} />
              </Row>
            </GroupCard>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, margin: "12px 0 4px" }}>
          ...
          <button onClick={() => setView(function(v) { return v === "schedule" ? "all" : "schedule"; })} style={{
